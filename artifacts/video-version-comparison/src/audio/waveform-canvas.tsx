...
// One version's waveform, tinted per analysis window by its diff class. The
// canvas redraws on every prop change so the playhead cursor tracks playback —
// this is the "wave scanner" the page is built around. Click anywhere to seek.
export function WaveformLane({
  ...
  const draw = useCallback(() => {
    ...
    // Playhead cursor — the scanner line.
  ...
  // Redraw whenever the props change (playhead moves, analysis recomputes).
  useEffect(() => {
    draw();
  }, [draw]);
...
// of windows, then CSS upscales. Redraws on playhead changes like the lanes.
export function SpectralMap({ result, playhead, onSeek }: SpectralProps) {
  ...
  const draw = useCallback(() => {
    ...
    for (const hz of [100, 1000, 10000]) {
      if (hz > result.bands[result.bands.length - 1].endHz) continue;
      const t = Math.log(hz / result.bands[0].startHz) / Math.log(result.bands[result.bands.length - 1].endHz / result.bands[0].startHz);
